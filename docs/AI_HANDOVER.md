...
Project name: TV Time Dashboard / Cinema Command Center.

Purpose: preserve TV Time watch history and turn the poster dashboard into an invite-only authenticated app with per-user isolated media libraries.

Current version: frontend `package.json` is `0.0.0`; UI footer says `v1.0.0`; backend is Laravel Phase 1 plus the one-day backend connection sprint. No release tag yet.
...
│   ├── README.md
│   ├── app/
│   │   ├── Console/Commands/ImportTvTimeUserCommand.php
│   │   ├── Enums/
│   │   ├── Filament/Resources/
...
7. Smoke test Basic Auth, Laravel login, `/api/v1/status`, `/api/v1/dashboard`, alert read/read-all, `/admin`.
